...
/**
 * Compiled Diagnostic Schema: Strict contract passed to the 7-seat Council
 */
export interface CompiledDiagnosticSchema {
  primary_friction: string; // Must be < 50 words
 ...
 */
export const AUSTERE_INTAKE_PROMPT = `[SYSTEM ROLE: AUSTERE INTAKE PARSER]
 ...
 */
export const COMPILATION_SYSTEM_PROMPT = `[SYSTEM ROLE: INTAKE COMPILATION PARSER]
 ...
 */
export async function compileDiagnosticSchema(
  history: IntakeMessage[],
  options: IntakeAgentOptions = {}
 ...
 */
async function generateAustereQuestion(
  history: IntakeMessage[],
  options: IntakeAgentOptions = {}
): Promise<string> {
  const apiKey = options.apiKey || process.env.GROQ_API_KEY;
 ...
 */
function simulateCompiledSchema(history: IntakeMessage[]): CompiledDiagnosticSchema {
